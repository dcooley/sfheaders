#ifndef R_SFHEADERS_MATRIX_SFC_H
#define R_SFHEADERS_MATRIX_SFC_H

#include "sfheaders/matrix/sfg.hpp"
...
namespace matrix {
  ...
  // }
  //
  // inline SEXP sfc_coordinates( Rcpp::List& sfc ) {
  //   // seprated this so it's independant / not called twice from `sf_to_df()`
  //   Rcpp::NumericMatrix sfc_coordinates = sfc_n_coordinates( sfc );
  //
  //   R_xlen_t n_geometries = sfc_coordinates.nrow();
  ...
  //   total_coordinates = total_coordinates + 1;
  //
  //   Rcpp::List res = get_sfc_coordinates( sfc, total_coordinates, false );
  //
  //   R_xlen_t i;
  //   Rcpp::NumericMatrix mat( total_coordinates, res.length() );
  //   //Rcpp::Rcout << "n_row: " << total_coordinates << std::endl;
  //
