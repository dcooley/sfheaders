...
test_that("holes are removed from polygons",{
  ...
  sfg <- sfg_multipolygon( obj = df, x = "x", y = "y", linestring_id = "ml_id", close = FALSE)
  ...
  expect_true( length( sfg[[1]] ) == 2 & length( sfg_res[[1]] ) == 1 )
  expect_equal( sfg_res[[1]][[1]][, 1], df[ df$ml_id == 1, "x"] )
  expect_equal( sfg_res[[1]][[1]][, 2], df[ df$ml_id == 1, "y"] )
