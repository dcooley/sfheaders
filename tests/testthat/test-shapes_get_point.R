context("shapes_get_point")

test_that("points extracted from different objects", {

  ### integer vector
  x <- 1L:2L
  res <- sfheaders:::rcpp_get_point(x, NULL)
  expect_equal( x, res )
  ...
  res <- sfheaders:::rcpp_get_point( x, c(0,1) )  ## c++ indexing
  ...
  expect_error(
    sfheaders:::rcpp_get_point( df, NULL )
    , "sfheaders - expecting single-row data.frame"
  )

  m <- as.matrix( df )
  expect_error(
    sfheaders:::rcpp_get_point( m, NULL )
