...
test_that("line matrices returend from various R objects",{
  ...
  x <- matrix(c(1L:4L), ncol = 2)
  res <- sfheaders:::rcpp_get_line( x, c(1,0) )  ## c++ indexed
  expect_equal( x[, c(2,1)], res )

  x <- matrix(c(1:4), ncol = 2)
  res <- sfheaders:::rcpp_get_line( x, c(0,1) )
  expect_equal( x, res )

  x <- data.frame(y = 1, x = 2)
  res <- sfheaders:::rcpp_get_line( x, c("x","y") )
  expect_equal( matrix( c(x$x, x$y), ncol = 2 ), res )

  x <- data.frame( y = 1:2, x = 3:4 )
  res <- sfheaders:::rcpp_get_line( x, c("y","x") )
  ...
  x <- data.frame( y = 1:2, x = 3:4 )
  res <- sfheaders:::rcpp_get_line( x, c(1,0) )
